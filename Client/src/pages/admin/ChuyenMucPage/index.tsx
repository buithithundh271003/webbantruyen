...
import { getAllChuyenMuc, removeChuyenMuc } from '../../../redux/Reducer/ChuyenMuc';
...
const ChuyenMucPage = () => {
    ...
    const categories = useAppSelector((state) => state.ChuyenMuc.chuyenmuc);

    useEffect(() => {
        setIsLoading(true);
        void dispatch(getAllChuyenMuc()).then(() => {
            setIsLoading(false);
        }).catch((error) => {
            setIsLoading(false);
            console.log(error);
    ...
    const confirm = async (id: string) => {
        await dispatch(removeChuyenMuc(id));
        messageApi.open({
            type: 'success',
